...
// Create lookup map by origin for efficient caching
const cachedCDMap = new Map<string, HistoryEntry>();
for (const cd of cachedCDs) {
  cachedCDMap.set(cd.origin, cd);
}
...
for await (const cd of new Pages().getPages()) {
  ...
  if (entry !== null) {
    // Check if we already have this entry cached with MusicBrainz data
    const cachedEntry = cachedCDMap.get(entry.origin);
    if (cachedEntry?.musicbrainz !== undefined) {
      ...
      const enrichedEntry = await mbClient.ensureMusicBrainzMetaData(entry);
